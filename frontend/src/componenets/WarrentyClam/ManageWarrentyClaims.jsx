...
function ManageWarrantyClaims() {
  ...
  const handleDelete = async (id) => {
    console.log("Deleting warranty claim with ID:", id); // Add this for debugging
    try {
      await axios.delete(`http://localhost:1010/public/warranty-items/${id}`);
      setWarrantyClaims(warrantyClaims.filter(claim => claim.id !== id));
    } catch (err) {
      setError('Failed to delete warranty claim');
    }
        ...
        <tbody>
          {warrantyClaims.map((claim) => (
            <tr key={claim.id}>
              ...
              <Link to={`/warranty-items/${claim.id}`}>
  <button className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition duration-200">
    Edit
  </button>
</Link>
