...
const ItemForm = () => {
    ...
    const [warrantyTime, setWarrantyTime] = useState(''); // New state for warrantyTime
    const [successMessage, setSuccessMessage] = useState('');
    const [errorMessage, setErrorMessage] = useState(''); // State for error message
        ...
        if (name === 'image') {
            ...
            switch (name) {
                ...
                case 'category':
                    setCategory(value);
                    break;
                case 'warrantyTime': // Handle warranty time input
                    setWarrantyTime(value);
                    break;
                default:
                    break;
            }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const itemData = {
                ...
                category,
                imageUrl, // Include image URL if uploaded
                warrantyTime // Include warranty time
            };
            ...
            setCategory('');
            setImage(null); // Clear image after submission
            setWarrantyTime(''); // Clear warranty time field
        } catch (error) {
            console.error("Error creating item listing:", error);
            setErrorMessage("There was an error creating the item listing.");
        }
        ...
        <div className="flex items-center justify-center min-h-screen">
            <form onSubmit={handleSubmit} className="p-6 rounded-lg shadow-lg border border-gray-600 w-full max-w-lg bg-gray-200">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-black">Warranty Time (months)</label>
                        <input
                            type="number"
                            name="warrantyTime"
                            value={warrantyTime}
